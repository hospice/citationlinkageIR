...
public class LinkageSearchWithLuceneAnalysisEvaluation22PapersA {
	...
	public Similarity getSimilarity(Algo algo){
		...
		case LMD:
			...
			//System.out.println("Mu: "+ similarityld.getMu());
			return similarityld;
		}
		return similarity;
	...
	public void runTest() throws Exception {
		...
		//System.out.println("Paper #" + " & " +"Category 1" + " & "+  "Category 2" + " & " + "Category 3"+ " & " +"Category 4"+ " & " +"Category 5"+"\\\\"+"\n");
		//System.out.println("\\hline"+"\n");
		
		
		stopSet.addAll(stopwords);

		BufferedWriter bw = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17_A/"+"evalPrec_.txt"));
		BufferedWriter bwdc = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17_A/"+"evaldc_.txt"));
		BufferedWriter bwScore = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17_A/"+"outScoreb_.txt"));
		BufferedWriter ExbwScore = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17_A/"+"expScore_.txt"));

		//BufferedWriter scoreAnalysis = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17/"+"analysisScoreNP_4.txt"));
		...
		//String dirF = "C:/Users/Hospice/Documents/boruta_data/linkageText/dataperfileIRPart/";
			...
			for(int k = 0; k < 6; k++){
				...
				System.out.println("Paper #" + " & " +"Category 1" + " & "+  "Category 2" + " & " + "Category 3"+ " & " +"Category 4"+ " & " +"Category 5"+"\\\\"+"\n");
				System.out.println("\\hline"+"\n");
				BufferedWriter scoreAnalysis = new BufferedWriter(new FileWriter("C:/Users/Hospice/Documents/boruta_data/linkagedata12_12/IR_01_17_A/"+"analysisScoreNP_"+k+".txt"));
				int t = 0;
				...
				for(File file:listFromFile){
					...
					ArrayList<Float> rating4 = new ArrayList<Float>();
					ArrayList<Float> rating5 = new ArrayList<Float>();
					t++;
				...
				//String citationText = computeNP(sentencesL[0].split("\t")[0].replace("_", " "));
				...
				for(int i=0; i <= sentencesL.length-1; i++){
                    ...
                    if(lineL.length!=0){
					String textL = lineL[1].replaceAll("(^.*[^\\.?!])\r?\n", " ").replaceAll("  +", " ");
					...
					if(Integer.valueOf(lineL[2])>0)
					{
						...
						if (Integer.valueOf(lineL[2])==1){
							one++;
							
						}
						if (Integer.valueOf(lineL[2])==2){
							two++;
							
						}
						if (Integer.valueOf(lineL[2])==3){
							three++;
							
						}
						if (Integer.valueOf(lineL[2])==4){
							four++;
							
						}
						if (Integer.valueOf(lineL[2])==5){
							five++;
				...
				for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
					...
					//if(arrayLMap.get(doc.get("content"))>0){
						...
						if((double)arrayLMap.get(doc.get("content"))==1.0){
							rating1.add((float) docId);
						}
						if((double)arrayLMap.get(doc.get("content"))==2.0){
							rating2.add((float) docId);
						}
						if((double)arrayLMap.get(doc.get("content"))==3.0){
							rating3.add((float) docId);
						}
						if((double)arrayLMap.get(doc.get("content"))==4.0){
							rating4.add((float) docId);
						}
						if((double)arrayLMap.get(doc.get("content"))==5.0){
							rating5.add((float) docId);
						}
				...
				System.out.println(t+"&"+rating1.size()+"/"+one+"& "+rating2.size()+"/"+two+" &"+rating3.size()+"/"+three+" &"+rating4.size()+"/"+four+"& "+rating5.size()+"/"+five+"\\\\\n\\hline");
				oneT+=rating1.size();
				twoT+=rating2.size();
				threeT+=rating3.size();
				fourT+=rating4.size();
